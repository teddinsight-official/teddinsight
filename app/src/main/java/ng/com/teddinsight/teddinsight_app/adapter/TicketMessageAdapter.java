...
public class TicketMessageAdapter extends RecyclerView.Adapter<TicketMessageAdapter.MessageViewHolder> {
    ...
    private List<TicketMessages> mMessageList;
    private DatabaseReference mUserDatabase;

    private final int LAYOUT_TYPE_ME = 1;
    private final int LAYOUT_TYPE_OTHERS = 2;

    public TicketMessageAdapter(List<TicketMessages> mMessageList) {
    ...
    public int getItemViewType(int position) {
        TicketMessages m = mMessageList.get(position);
    ...
    public void onBindViewHolder(final MessageViewHolder viewHolder, int i) {
        ...
        TicketMessages c = mMessageList.get(i);
    ...
    public void addAll(List<TicketMessages> newMessages) {
        int initialSize = mMessageList.size();
        mMessageList.addAll(newMessages);
        notifyItemRangeInserted(initialSize, newMessages.size());
    }

    public String getLastItemId() {
        return mMessageList.get(mMessageList.size() - 1).messageId;
    }
}
