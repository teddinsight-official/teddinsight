...
public class ClientCalendarViewModel extends ViewModel {
    ...
    private MyValueEventListenerClass myValueEventListener;
    ...
    public void addDummyCalendar(){
        ...
        ClientCalendar c = new ClientCalendar();
        c.setName("cal");
        c.setBeginPublishing(false);
        c.setNeedsPublishing(true);
        c.setTaskCount(5);
        reference.child(key).setValue(c);
    }

    class MyValueEventListenerClass implements ValueEventListener {
        @Override
    ...
    public ClientCalendarViewModel() {
        myValueEventListener = new MyValueEventListenerClass();
        reference = FirebaseDatabase.getInstance().getReference().child(ClientCalendar.getBaseTableName());
        query = reference;
        _message.setValue(null);
